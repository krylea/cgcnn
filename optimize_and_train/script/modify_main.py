# changes to main.py
# 1. Delete CIFData to load data in main function
# 2. Save epoch, mae and loss
# 3. Change name based on train size
...
from modify_data import collate_pool, get_train_val_test_loader
...
def main_cgcnn(args, dataset):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_mae, train_loss = train(args, train_loader, model, criterion, optimizer, epoch, normalizer)
...
def validate(args, val_loader, model, criterion, normalizer, outdir, test=False):
    ...
    if args.task == 'regression':
        print(' {star} MAE {mae_errors.avg:.3f}'.format(star=star_label,
                                                        mae_errors=mae_errors))
        #### change: return valid/test mae and loss
